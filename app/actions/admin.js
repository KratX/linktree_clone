...
// FIX: New action to get a specific user's profile and all their links
export async function getAdminUserDetails(userId) {
    ...
    const links = await db.collection("links").find({ userId: userId }).sort({ order: 1 }).toArray();
...
// FIX: New action to edit a link as an admin
export async function adminEditLinkAction(linkId, formData) {
    await checkAdmin();
    const client = await clientPromise;
    const db = client.db();
