        ...
        <div className="mx-auto max-w-4xl text-center">
          ...
          <div className="mx-auto mt-8 inline-flex items-center gap-3 rounded-full border border-slate-200 bg-white p-1.5">
            <button
              ...
              className={`rounded-full px-4 py-2 text-sm font-semibold transition ${!annual ? "bg-black text-white" : "text-slate-500"
                }`}
            >
              Monthly
            </button>
            <button
              ...
              className={`flex items-center gap-2 rounded-full px-4 py-2 text-sm font-semibold transition ${annual ? "bg-black text-white" : "text-slate-500"
                }`}
            >
              Annual
      ...
      <section className="px-6 py-16 md:px-10 lg:px-16">
        <div className="mx-auto grid max-w-7xl gap-6 lg:grid-cols-4">
          {PLANS.map((plan) => (
            <div
              key={plan.name}
              className={`flex flex-col rounded-3xl border p-7 ${plan.highlighted
            ...
            >
              ...
              <p
                className={`mt-1 text-sm ${plan.highlighted ? "text-slate-400" : "text-slate-500"}`}
              >
                {plan.tagline}
              </p>
              ...
              {plan.monthly > 0 && (
                <p
                  className={`mt-1 text-xs ${plan.highlighted ? "text-slate-500" : "text-slate-400"}`}
                >
              ...
              <Link
                ...
                className={`mt-6 rounded-full px-5 py-3 text-center text-sm font-semibold transition ${plan.highlighted
              ...
              <ul className="mt-7 space-y-3">
                {plan.features.map((f) => (
                  <li
                    key={f}
                    className={`flex items-start gap-2 text-sm ${plan.highlighted ? "text-slate-300" : "text-slate-600"
                      }`}
                  >
                    <Check
                      size={15}
                      className="mt-0.5 shrink-0 text-[#0a5cfa]"
                    />
                    {f}
                  </li>
                ))}
            ...
            {FAQS.map((faq, i) => {
              ...
              return (
                <div key={faq.q}>
                  ...
                  {isOpen && (
                    <p className="px-6 pb-5 text-sm leading-6 text-slate-500">
                      {faq.a}
                    </p>
                  )}
                </div>
